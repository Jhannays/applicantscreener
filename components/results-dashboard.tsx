...
interface ResultsDashboardProps {
  ...
  /** AI-generated guided questions for SME */
...
export function ResultsDashboard({
  ...
  const generateModelPrompt = () => {
    ...
    // Section 5: Guided SME Q&A
    const answeredQuestions = smeQuestions.filter((q) => q.answer);
    prompt += "---\n\n## 5. Guided SME Q&A\n\n";
    prompt += "The following questions were generated by AI based on ambiguities in the screening results, then answered by a domain Subject Matter Expert. Each answer represents a rule the model MUST follow.\n\n";

    if (answeredQuestions.length > 0) {
      // Group by category
      const byCategory = new Map<string, typeof answeredQuestions>();
      ...
      for (const [category, qs] of byCategory) {
        ...
        for (const q of qs) {
          ...
          prompt += `**A (SME):** ${q.answer}\n\n`;
        }
      }
    } else {
      prompt += "_No guided questions were answered._\n\n";
    }

    // Include unanswered questions as areas needing further review
    const unanswered = smeQuestions.filter((q) => !q.answer);
    if (unanswered.length > 0) {
      prompt += "### Unanswered Questions (areas still needing SME clarification)\n\n";
    ...
    prompt += `- Guided questions generated: ${smeQuestions.length}\n`;
    prompt += `- Guided questions answered: ${answeredQuestions.length}\n`;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            disabled={smeSessionNotes.length === 0 && roleNotes.length === 0 && correctionRules.length === 0 && smeQuestions.filter((q) => q.answer).length === 0}
